...
from centerDetection import getCenter
import threading
import sounddevice as sd
from geometry_utils import calculate_distance_and_angle
from visualization1 import draw_path, log_pink_circle_coordinates, update_pink_circle_coordinates, save_series_shots, calculate_score, calculate_aim_trace_speed, calculate_inside_ring_avg
...
# container
    async def main():
        result = await getCenter(resized_frame) 
        return result
    ...
    for run_num in range(1):  # Set the number of runs here
        ...
        out_tracking = cv2.VideoWriter(f'Output/frame_tracking_{ind}.mp4', fourcc_tracking, frame_rate, (resized_frame.shape[1], resized_frame.shape[0]))

        # Define the codec and create a video writer object for path visualization
        fourcc_path = cv2.VideoWriter_fourcc(*'H264')
        out_path = cv2.VideoWriter(f'Output/path_visualization_{ind}.mp4', fourcc_path, frame_rate, (path_visualization.shape[1], path_visualization.shape[0]))
                            
        # Define the log file name
        log_filename = f'Output/distance_angle_log_{ind}.csv'
    ...
    # line break
        ...
        pink_circle_coordinates = list(zip(df['X'], df['Y']))
        save_series_shots(pink_circle_coordinates, final_bg)
    
    
    
with stats:
    tab, video, image1 = st.columns([5.75,.25,3])
    
    with tab:
        st.table(df)
        if st.button("Series shot"):
            # Store the selected video path in a SessionState variable
            st.session_state.selected_shot = 'Output/series_shots.png'

        if st.button("Reset and Close"):
            filename = "Output\pink_circle_coordinates.csv"
            with open(filename, 'w') as file:
            # Write a header or any initial content if needed
                file.write("ind,X,Y,Direction,Score,Aim Time,10.0,S1,S2,DA,10a.0\n")
                
            directory_path = 'Output'
            files = os.listdir(directory_path)
            # Prefix to match for deletion
            del1 = 'distance_angle'
            del2 = 'frame_tracking'
            del3 = 'path_visualization'
            del4 = 'serieis'
            files_to_delete = [del1, del2, del3, del4]

            for file_name in files:
                if any(file_name.startswith(file) for file in files_to_delete):
    ...
    with video:
        st.markdown("""
            ...
            """, unsafe_allow_html=True)  # Apply CSS before creating columns
        st.write("⏯")
    ...
    with image1:
        ...
        selected_shot = getattr(st.session_state, 'selected_shot', None)

        if selected_shot:
            st.image(selected_shot)
